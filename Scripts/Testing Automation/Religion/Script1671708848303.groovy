...
if(Mobile.waitForElementPresent(Back, 30)) {
	...
	if(Mobile.waitForElementPresent(Book, 30)) {

		KeywordUtil.markPassed('PASSED: Book icon present on the screen')
		...
		if(Mobile.waitForElementPresent(What, 30)) {
			...
			CustomKeywords.'com.spotlove.Utility.random_value'(Religion)
		...
		KeywordUtil.markFailed('FAILED: Book icon is not present on the screen')
	}
}else {
