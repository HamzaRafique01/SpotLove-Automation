...
if(Mobile.waitForElementPresent(Back, 30)) {
	...
	if(Mobile.waitForElementPresent(Mirror_icon, 30)) {

		KeywordUtil.markPassed('PASSED: Mirror Icon present on the screen')

		if(Mobile.waitForElementPresent(How_do, 30)) {

			KeywordUtil.markPassed('PASSED: How do... label text present on the screen')

			CustomKeywords.'com.spotlove.Utility.random_value'(Gender)
			...
			KeywordUtil.markFailed('FAILED: How do... label text is not present on the screen')
		}
	}else {
		KeywordUtil.markFailed('FAILED: Mirror Icon is not present on the screen')
	}
}else {
