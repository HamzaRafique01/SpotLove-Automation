...
if (Mobile.waitForElementPresent(hearticon, 30)) {
	...
	if(Mobile.waitForElementPresent(Meet_your, 30)){
		
		KeywordUtil.markPassed('PASSED: Meet your label text is present on the screen')
		
		if(Mobile.waitForElementPresent(Soulmate, 30)){
			
			KeywordUtil.markPassed('PASSED: Soulmate label text is present on the screen')
			
			if(Mobile.waitForElementPresent(Plenty, 30)) {
			
				KeywordUtil.markPassed('PASSED: Plenty.... label text is present on the screen')
				...
				KeywordUtil.markFailed('FAILED: Plenty.... label text is not present on the screen')
			}
		}else {
			KeywordUtil.markFailed('FAILED: Soulmate label text is not present on the screen')
		}
	}else {
		KeywordUtil.markFailed('FAILED: Meet your label text is not present on the screen')
	}
}else {
